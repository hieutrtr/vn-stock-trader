...
description: Phân tích cơ bản một mã cổ phiếu Việt Nam — định giá, tài chính, triển vọng ngành
triggers:
  - "phân tích cơ bản"
  - "FA"
  - "PE ratio"
...
```
get_company_overview(symbol)           → tên, ngành, vốn hoá, sàn
get_financial_statements(symbol)       → BCTC 4 quý gần nhất
get_financial_ratios(symbol)           → P/E, P/B, ROE, ROA, EPS
get_industry_data(industry)            → P/E ngành, tăng trưởng ngành
```
