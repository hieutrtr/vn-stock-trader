...
- **KHÔNG dùng markdown link** `[text](url)` — Telegram không render được. PHẢI paste plain text URL trực tiếp cho mỗi tin/bài viết (VD: `https://cafef.vn/article/123`). Chỉ cấm cú pháp `[text](url)`, KHÔNG cấm hiện URL.
...
- **Ngôn ngữ output:** Khi viết tiếng Việt, LUÔN LUÔN phải có dấu đầy đủ (ví dụ: "Thị trường chứng khoán" chứ KHÔNG ĐƯỢC viết "Thi truong chung khoan"). Không bao giờ viết tiếng Việt không dấu.
